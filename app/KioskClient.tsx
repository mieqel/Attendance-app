...
export default function KioskClient({
  ...
  const [toast, setToast] = useState<{ patient: Patient; checkedIn: boolean } | null>(null);
  const [editing, setEditing] = useState<Patient | null>(null);
  const [registering, setRegistering] = useState(false);
  const [droppingIn, setDroppingIn] = useState(false);
  const [isPending, startTransition] = useTransition();

  // One tap toggles check-in: tap once to check in, tap again to undo.
  // No confirmation step in either direction — simplest possible interaction
  // for the shared tablet.
  function handleTap(p: Patient) {
    const nextCheckedIn = !p.checkedIn;
    setPatients((prev) => prev.map((x) => (x.id === p.id ? { ...x, checkedIn: nextCheckedIn } : x)));
    setToast({ patient: p, checkedIn: nextCheckedIn });
    startTransition(async () => {
      const res = nextCheckedIn ? await checkInPatient(p.id) : await checkOutPatient(p.id);
      if (!res.ok) {
        setPatients((prev) => prev.map((x) => (x.id === p.id ? { ...x, checkedIn: !nextCheckedIn } : x)));
      }
    });
  ...
  function handleDropIn(p: Patient) {
    ...
    setDroppingIn(false);
    setToast({ patient: p, checkedIn: true });
    startTransition(async () => {
    ...
    <main className="flex-1 flex flex-col px-4 py-6 md:px-10 md:py-8">
      <header className="text-center mb-6 md:mb-10">
        ...
        <p className="text-lg text-ink-muted mt-2">
          Tik op jouw figuur om in te checken. Nogmaals tikken maakt het ongedaan.
        </p>
      </header>
      ...
      {toast && (
        <div className="fixed inset-x-0 top-6 flex justify-center z-50 pointer-events-none px-4">
          <div className="bg-teal text-white rounded-2xl px-6 py-4 shadow-xl flex items-center gap-3 max-w-sm">
            <AvatarSvg
              skinTone={toast.patient.skinTone}
              hairStyle={toast.patient.hairStyle}
              hairColor={toast.patient.hairColor}
              seed={toast.patient.id}
              size={48}
            />
            <p className="font-display text-lg font-semibold">
              {toast.checkedIn
                ? `Leuk dat je er bent, ${toast.patient.name}!`
                : `${toast.patient.name}, tot de volgende keer!`}
            </p>
          </div>
        </div>
      )}
